...
class ModelSaleOrder extends Model {
	public function isOrderEvent($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id . "'");

		foreach($query->rows as $order) {
			$product_id = $order['product_id'];

			$query_1 = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
  
			foreach($query_1->rows as $product) 
			  if($product['type'] == 1)
				return true;
		}
	...
	public function isIncludeTax($order_id) {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, o.order_status_id, o.invoice_no, o.invoice_coupon, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS order_status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
	...
	public function createInvoiceNo($order_id) {
		...
		if ($order_info && !$order_info['invoice_no']) {
			$query = $this->db->query("SELECT MAX(invoice_no) AS invoice_no FROM `" . DB_PREFIX . "order` WHERE 1");
