...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "category SET type = '".(int)$data['category_description'][3]['type'] ."', parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), date_added = NOW()");
	...
	public function getCategories($data = array()) {
		...
		if($type == 1)
		  $sql .= " AND c1.type = 1 AND c1.parent_id >0 ";
		elseif($type == 2)
		  $sql .= " AND c1.type = 1 ";
		elseif($type == 3)
		  $sql .= " AND c1.type = 1 ";
		else        
		  $sql .= " AND c1.parent_id = 0 OR c1.type = 0";
