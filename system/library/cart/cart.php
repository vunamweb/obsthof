...
class Cart {
	...
	public function __construct($registry) {
		...
		if ($this->customer->getId()) {
			...
			foreach ($cart_query->rows as $cart) {
				...
				$this->add($cart['product_id'], $cart['quantity'], json_decode($cart['option']), $cart['recurring_id'], $cart['idOption'], 0, $cart['valueCoupon']);
			}
		}
	}
	...
	public function getProducts() {
		...
		$_SESSION['session_id'] = $this->session->getId();
		$_SESSION['api_id'] = $this->session->data['api_id'];
		$_SESSION['customer_id'] = $this->customer->getId();
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
						...
						if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio') {
							...
							if ($option_value_query->num_rows) {
								...
								$option_data[] = array(
									...
									'price_1'                 => $option_value_query->row['price_1'],
				...
				$timeOption = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option WHERE product_option_id = '" . $cart['idOption']. "'");
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0, $idOption = 0, $child = 0, $valueCoupon = 0) {
		...
		if (!$query->row['total']) {
			...
			//echo $valueCoupon . 'uodate'; die();
			if($valueCoupon == 0)
			  $this->db->query("UPDATE " . DB_PREFIX . "cart SET quantity = (quantity + " . (int)$quantity . ") WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND idOption = '" . (int)$idOption . "' AND child = '" . (int)$child . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");
			else
			$this->db->query("UPDATE " . DB_PREFIX . "cart SET valueCoupon = ".$valueCoupon." WHERE api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND idOption = '" . (int)$idOption . "' AND child = '" . (int)$child . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");
		    }
	}
