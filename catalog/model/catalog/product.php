    ...
    public function getProduct( $product_id ) {
        ...
        if ( $query->num_rows ) {
            return array(
                ...
                'price_1'            => ( $query->row[ 'discount' ] ? $query->row[ 'discount' ] : $query->row[ 'price_1' ] ),
                'price_lit'            => ( $query->row[ 'discount' ] ? $query->row[ 'discount' ] : $query->row[ 'price_lit' ] ),
...
$query = $this->db->query( 'select * from ' . DB_PREFIX . 'product where type = 1' );
            ...
            foreach ( $query->rows as $result ) {
                ...
                foreach ( $options as $item ) {
                    ...
                    //$date = $value[0] . ' ' . $value[1] . '-' . $value[2] . 'Hours';
                    $date = explode('-', $value[0]);

                    $date = $date[2] . '-' . $date[1] . '-' . $date[0];
                    $date_1 = $date;
                    $short_date = $date . ' ' . explode(':', $value[1])[0] . ':' . explode(':', $value[2])[0];
						
                    $format = "day month date";

                    $date = $this->document->formatDate($date, $format);
                    $date .= ' ' . $value[1] . '-' . $value[2] . ' Uhr';
                    //$date = $this->document->formatDate('2023-08-28', 'day month date');

                    if(time() - strtotime($date_1) <=0) {
                        $data[] = $this->getProduct( $result[ 'product_id' ] );
                        $data[count($data)-1]['date_event'] = $date;
                        $data[count($data)-1]['product_option_id'] = $item['product_option_id'];
                        $data[count($data)-1]['short_date'] = $short_date;
                    }
                }
            }
    ...
    public function getDateEvent( $option_id ) {
        ...
        return ($value[3] > 0) ? $date1_ . ', ' . $value[ 1 ] . '-' . $value[ 2 ] . ' Uhr' . '<br><p class="lead">'.$value[ 3 ].' Plätze</p>' : 
        $date1_ . ', ' . $value[ 1 ] . '-' . $value[ 2 ] . ' Uhr<br><p></p>';
        ;
