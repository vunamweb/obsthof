...
class ControllerCheckoutCart extends Controller {
    public function index() {
        ...
        if ( $this->cart->hasProducts() || !empty( $this->session->data[ 'vouchers' ] ) ) {
			...
			//print_r($this->session->data[ 'data_option' ]);
			if(!$this->model_checkout_order->getStatusValueTicket()) {
            ...
            if ( $this->customer->isLogged() || !$this->config->get( 'config_customer_price' ) ) {
                ...
                if($totalNormalProduct < $this->config->get('config_login_attempts'))
                  $data['text_warning_ship'] = $this->language->get( 'text_warning_ship' );
        ...
        public function add() {
            ...
            if ( isset( $this->request->post[ 'data_option' ] ) ) {
                $data_option = ( int )$this->request->post[ 'data_option' ];
            } else {
                $data_option = 0;
            }
            ...
            if ( $product_info ) {
                if ( isset( $this->request->post[ 'quantity' ] ) && !isset($_REQUEST['child']) ) {
                    //echo $_REQUEST['child']; die(); 
                    $quantity = ( int )$this->request->post[ 'quantity' ];
                } else if(isset($_REQUEST['child'])) {
                    $quantity = ( int )$this->request->post[ 'quantity_1' ];
                    //echo $quantity;die();
                } else {
                    $quantity = 1;
                    //echo '11'; die();
                ...
                if ( !$json ) {
                    ...
                    if($this->model_checkout_order->getStatusValueTicket($data_option, $quantity)) {
                        ...
                        //echo '1'; die();
                        $json[ 'error' ][ 'recurring' ] = 'unzureichende Anzahl an Tickets'; //$this->language->get( 'error_recurring_required' );
                    }
                    // if case of option
					/*if ( $data_option > 0 ) {
                        // if ready save option in session
                        if ( isset($this->session->data[ 'data_option' ]) ) {
                            $dataOption = $this->session->data[ 'data_option' ];
                            $noExist = true;

                            for ( $i = 0; $i < count( $dataOption );
                            $i++ )
                            if ( $dataOption[ $i ][ 'option' ] == $data_option ) {
                                $dataOption[ $i ][ 'value' ] = $dataOption[ $i ][ 'value' ] + $quantity;
                                $noExist = false;
                            }

                            if ( $noExist ) {
                                $count = count( $dataOption );
                                $dataOption[ $count ][ 'option' ] = $data_option;
                                $dataOption[ $count ][ 'value' ] = $quantity;

                            }

                            $this->session->data[ 'data_option' ] = $dataOption;
                        }

                        // if is first time to save in session
                        else {
							$array = array();
							$array[0]['option'] = $data_option;
							$array[0]['value'] = $quantity;
							
							//array( 'option' => $data_option, 'value' => $quantity );
                            $this->session->data[ 'data_option' ] = $array;
                        }
                    } */
            ...
            public function remove() {
                ...
                if ( isset( $this->request->post[ 'key' ] ) ) {
                    ...
                    unset( $this->session->data[ 'payment_methods' ] );
					unset( $this->session->data[ 'reward' ] );
					unset($this->session->data['data_option']);
